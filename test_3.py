...
def hash_predict(data, divide0, divide1, robot_final_0p1r, robot_final_1p0r):
    search1 = joblib.load("kmeans-2")
    kmeans_yred = Kmeans_predict(search1, data)
    hash_ypred = []
    for i in range(data.shape[0]):
        if kmeans_yred[i] == 0:
            hash_code = get_hash_code(divide1, data[i])
            j = judge_hash(robot_final_1p0r, hash_code)
        else:
            hash_code = get_hash_code(divide0, data[i])
            j = judge_hash(robot_final_0p1r, hash_code)
        hash_ypred.append(j)
    return hash_ypred
